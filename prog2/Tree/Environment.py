...
class Environment(Node):
    ...
    def define(self, id, value):
        ...
        fval = Environment.__find(id, self.frame)
        preframe = Cons(id, Cons(value, Nil.getInstance()))
        if fval == None:
            self.frame = Cons(preframe, self.frame)
        else:
            fval.setCar(value)
    ...
    def assign(self, id, value):
        # TODO: implement this function
        fval = Environment.__find(id, self.frame)
        if fval == None and self.env == None:
            self._error("undefined variable " + id.getName())
        elif fval == None:
            # assign identifier in enclosing scope
            self.env.assign(id, value)
        else:
            # set the value out of the list we got from find()
            fval.setCar(Cons(id, value))

    @classmethod    
    def populateEnv(self, env, filename):
        # TODO: implement this function
        builtf = ['b+', 'b>', 'b-', 'b*', 'b/', 'b=', 'b<', 'symbol?', 'number?', 'car', 'cdr', 'cons', 'set-car!', 'set-cdr!', 'null?', 'pair?', 'eq?', 'procedure?', 'read', 'write', 'display', 'newline', 'eval', 'apply', 'interaction-environment', 'load']
        for func in builtf:
            fname = Ident(func)
            env.define(fname, BuiltIn(fname))
        #BuiltIn load can generate parse tree from ini.scm, store filename in car, nil in cdr
        loader = env.lookup(Ident('load'))
        loader.apply(Cons(StrLit(filename), Nil.getInstance()))
